...
import it.ute.QAUTE.entity.EventRegistration.RegistrationStatus;
...
public class EventService {
    ...
    public EventRegistration registerForEvent(Integer eventId, User user, String note) {
        ...
        if (registrationRepository.existsByEventAndUser(event, user) && 
                registrationRepository.findByEventAndUser(event, user).get()
                .getStatus() == EventRegistration.RegistrationStatus.Registered) {
    ...
    public EventRegistration updateRegistrationStatus(Integer registrationId,
        ...
        EventRegistration registration = registrationRepository.findById(registrationId)
                .orElseThrow(() -> new AppException(ErrorCode.REGISTRATION_NOT_FOUND));
        if (registration.getEvent().getConsultant().getConsultantID() != consultant.getConsultantID()) {
            throw new AppException(ErrorCode.UNAUTHORIZED);
        }
        Event event = registration.getEvent();
        LocalDateTime now = LocalDateTime.now();

        if ((newStatus == EventRegistration.RegistrationStatus.Attended
                || newStatus == EventRegistration.RegistrationStatus.Absent)
        ...
        EventRegistration updatedReg = registrationRepository.save(registration);
        log.info("Updated registration {} status to {} by consultant {}", registrationId, newStatus,
                consultant.getConsultantID());

        if (newStatus == EventRegistration.RegistrationStatus.Confirmed) {
        notificationService.notifyUserRegistrationConfirmed(registration.getUser(),
        event);
        }
        return updatedReg;
    }
